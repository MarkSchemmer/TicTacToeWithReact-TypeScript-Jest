...
describe(' simulating clicks on board and on history buttons ', () => {
    ...
    it(' click board 3 time should have 4 buttons for history ', () => {
        ...
        expect(wrapper.find('.hist-btn')).toHaveLength(5)
    })
    ...
    */
    ...
    it(' Please refer to comment statement P ', () => {
       ...
       //  expect(findSquares.everyWhere(x => x.text() )).toEqual('')
        ...
        expect(wrapper.find('.hist-btn').at(2).html()).toEqual("<button class=\"hist-btn\">  Player: O Move :2 Coordinate : (0, 1)  </button>")

        wrapper.find('.hist-btn').at(2).simulate('click')
        wrapper.update()

        expect(wrapper.find('.hist-btn').at(2).html()).toEqual("<button class=\"hist-btn\"><strong> Player: O Move :2 Coordinate : (0, 1) </strong></button>")
        ...
        expect(findUpdatedSquares.at(5).text()).toEqual('')
